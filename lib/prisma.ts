...
if (databaseUrl) {
  ...
  if (isPooler) {
    ...
    if (databaseUrl.includes(':5432')) {
      console.warn('⚠️  DATABASE_URL: Using pooler hostname with direct port (5432). Fixing to use pooler port (6543)...')
    ...
    if (!databaseUrl.includes('pgbouncer=true')) {
      ...
      console.log('✅ Added pgbouncer=true to connection string to disable prepared statements')
    }
  }
  ...
  if (process.env.NODE_ENV === 'development' && databaseUrl !== process.env.DATABASE_URL) {
...
if (databaseUrl !== process.env.DATABASE_URL) {
  // If we modified the URL, set it in the datasources config
...
if (databaseUrl !== process.env.DATABASE_URL && globalForPrisma.prisma) {
  console.log('🔄 Clearing existing Prisma client to apply new connection string...')
